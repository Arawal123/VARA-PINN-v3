...
from scripts.run_modern_baselines import STEADY_METHODS
...
def test_uvp_soft_bc_formulation_returns_raw_fields_and_residuals():
    ...
    base["cavity_base_formulation"] = "uvp_soft_bc"
...
def test_reliable_uvp_defaults_preserve_budgets_and_reference_free_checkpointing():
    ...
    for preset, expected in (("diagnostic", 2000), ("reliable", 4000)):
        ...
        assert not trainer._compute_boundary_training_loss(
            config["training"]["weights"]
        )


def test_cavity_uvp_velocity_lift_enforces_walls_and_keeps_direct_uvp():
    class RawUVP(torch.nn.Module):
        def forward(self, coords):
            return torch.ones((coords.shape[0], 3), dtype=coords.dtype)
    ...
    assert output[:, 1].abs().max().item() < 1e-12
    assert output[0, 0].item() == pytest.approx(1.0)
    assert output[1:, 0].abs().max().item() < 1e-12
...
def test_lifted_uvp_regional_losses_use_only_pde_residuals():
    class LiftedModel:
        physics_formulation = "cavity_uvp_velocity_lift"

    model = LiftedModel()
    coords = torch.tensor(
        [[0.5, 0.8], [0.82, 0.5], [0.5, 0.2]],
        dtype=torch.float64,
    )
    residuals = {
        "coords": coords,
        ...
        "f_c": torch.tensor([[0.2], [0.2], [0.2]], dtype=torch.float64),
    ...
    assert pointwise["top_band_momentum_u"].item() == pytest.approx(1.0)
    assert pointwise["right_wall_interior_pde"].item() == pytest.approx(4.29)
    assert pointwise["lower_core_pde"].item() == pytest.approx(9.29)
...
def test_adaptive_baselines_build_complete_sparse_polish_losses(
    tmp_path,
    method,
):
    config = deep_update(
    ...
    pointwise, _ = trainer._build_pointwise_losses(
        batch,
        dict(trainer.config["training"]["weights"]),
    )
    required = {
        "bc_uvp_balanced",
        "cfd_u_mse",
    ...
    assert required.issubset(pointwise)


def test_sparse_polish_baseline_fix_does_not_mutate_safe_method_configs(
    tmp_path,
):
    config = _small_cavity_trainer_config(
        tmp_path,
        data_supervision_mode="sparse_cfd_polish",
    )
    config = deep_update(
        ...
        {
            "experiments": {
                "root": str(tmp_path / "safe_methods"),
                "flat_layout": True,
            }
        },
    )
    expected = deepcopy(config)
    vanilla = _trainer_for("vanilla", deepcopy(config))
    rar = _trainer_for("rar", deepcopy(config))
    vara = VARAV2Trainer(
        deep_update(
            deepcopy(config),
            {
    ...
    assert vanilla.config == expected
    assert rar.config["training"] == expected["training"]
    assert rar.config["data_supervision"] == expected["data_supervision"]
    assert vara.config["training"] == expected["training"]
    assert vara.config["data_supervision"] == expected["data_supervision"]


def test_shared_pointwise_helper_matches_existing_vanilla_sparse_polish_call(
    tmp_path,
):
    config = deep_update(
        _small_cavity_trainer_config(
        ...
        {
            "experiments": {
                "root": str(tmp_path / "vanilla_equivalence"),
                "flat_layout": True,
            }
        },
    )
    trainer = _trainer_for("vanilla", config)
    batch = trainer.initial_batch()
    weights = dict(trainer.config["training"]["weights"])
    residual_mode, residual_delta = trainer._residual_loss_settings()
    direct = compute_pointwise_losses(
        trainer.model,
        batch,
        trainer.benchmark,
        trainer.steady,
        ...
        compute_boundary_loss=trainer._compute_boundary_training_loss(
            weights
        ),
        runtime_profile=None,
    )
    shared, _ = trainer._build_pointwise_losses(batch, weights)
    assert direct.keys() == shared.keys()
    for name in direct:
        assert torch.equal(direct[name], shared[name]), name


def test_re100_reference_and_steady_causal_template_remain_safe():
    reference_map = _load_full_field_reference_map(
    ...
    assert reference.as_posix().endswith(
        "data/references/lid_driven_cavity/full_field/"
        "re_0100_paddlescience.npz"
    )
    assert "causal" not in STEADY_METHODS


def test_effective_loss_diagnostics_capture_constructed_keys_and_weights(
    tmp_path,
):
    config = deep_update(
        _small_cavity_trainer_config(
    ...
    trainer = _trainer_for("gradient_balanced", config)
    batch = trainer.initial_batch()
    pointwise, _ = trainer._build_pointwise_losses(
        batch,
        dict(trainer.config["training"]["weights"]),
    )
    losses = compute_global_losses(pointwise, reduction="mean")
    trainer._record_effective_loss_diagnostics(
        losses,
        dict(trainer.config["training"]["weights"]),
    )
    assert "bc_uvp_balanced" in trainer.effective_loss_keys
    assert "cfd_u_mse" in trainer.effective_loss_keys
    assert trainer.effective_loss_weights["bc_uvp_balanced"] > 0.0
    assert trainer.effective_loss_weights["cfd_v_mse"] > 0.0


@pytest.mark.parametrize(
    "method,runner_name",
    [
        ("gradient_balanced", "_train_gradient_balanced_epochs"),
        ("gradient_enhanced", "train_epochs"),
        ("relobralo", "_train_relobralo_epochs"),
        ("residual_attention", "_train_residual_attention_epochs"),
        ("self_adaptive_attention", "_train_self_adaptive_epochs"),
    ],
)
def test_adaptive_sparse_polish_one_step_smoke(
    tmp_path,
    method,
    runner_name,
):
    config = deep_update(
        ...
        {
            "experiments": {
                "root": str(tmp_path / method / "smoke"),
                "flat_layout": True,
            },
    ...
    trainer = _trainer_for(method, config)
    batch = trainer.initial_batch()
    runner = getattr(trainer, runner_name)
    runner(
        batch,
        None,
        cycle=0,
        epochs_override=1,
        log_prefix="wiring_smoke",
    )
    assert trainer.global_step == 1
    assert "bc_uvp_balanced" in trainer.effective_loss_keys
    assert "cfd_u_mse" in trainer.effective_loss_keys
    assert "cfd_v_mse" in trainer.effective_loss_keys
...
def test_uvp_formulation_is_identical_for_vanilla_and_vara(tmp_path):
    ...
    assert vanilla.model.physics_formulation == "cavity_uvp_velocity_lift"
