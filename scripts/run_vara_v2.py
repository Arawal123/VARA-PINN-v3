"""Run the isolated matched-compute VARA Controller V2."""
...
def main() -> None:
    ...
    parser.add_argument("--output_dir", default="experiments/vara_v2/lid_driven_cavity")
    ...
    for seed in args.seeds:
        ...
        metrics = trainer.run()
        rows.append({**metrics, "seed": int(seed), "run_dir": str(trainer.run_dir)})
        print(f"seed={seed}: {trainer.run_dir}")

    summary = Path(args.output_dir) / "summary"
    summary.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(rows).to_csv(summary / "vara_v2_raw.csv", index=False)
